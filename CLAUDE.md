...
- **Presence heartbeat** — on `sessionStart` only, both dispatchers additionally POST `{agent_family: "cursor", agent: "cursor", version, host, actor_*}` to `/api/v1/hooks/status` so the install shows in the dashboard's Coding Agents roster with a real version (from `plugin.json`) and hostname. Pure side-effect: response ignored, never affects the relayed hook response. `hook.sh` detaches it (double-fork) so session start never waits; `hook.ps1` runs it sync after `Emit-Json` (PowerShell has no reliable fire-and-forget across process exit). `agent` is the canonical `"cursor"`, not a display label — the server keys its latest-version lookup on it.
