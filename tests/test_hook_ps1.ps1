...
#   • POSIX single-quoting with `'\''` escapes ............ install.ps1
#   • bash `printf %q` (backslash + double-quote escapes) .. install.sh / compile
...
# (hook.ps1 stands down on non-Windows for its MAIN body, but the test only
#  loads its functions via the ROGUE_PS_LIB_ONLY seam, so this runs anywhere.)
...
# ── POSIX single-quoting (install.ps1 Format-EnvVal) ───────────────────────
...
# ── bash printf %q output (install.sh / compile-customer-plugin.sh) ────────
